...
export const BatteriesForFlightPlan = React.memo((props) => {
    ...
    async function fetchRecords() {
        try {
            ...
            if (response.data.total_records > 0) {
                setSelectionModel(() => {
                    return response.data.records.map((battery) => {
                        if (String(battery.id) === String(props.current.battery_id)) {
                            return battery.id;
                        }
                    });
                });
        ...
        const execute = async () => {
            ...
            const selectedFlightPlansUpdated = props.selectedFlightPlans.map((flight_plan) => {
                if (flight_plan.id != props.current.id) {
                    ...
                    return { ...flight_plan, battery_id: selectionModel }
                }
            });
